...
public class PlayerChunkViewer extends JFrame
{
    public static class TicketInfo
    {
        ...
        public TicketInfo(PacketCustom packet, WorldClient world)
        {
            ...
            type = net.minecraftforge.common.ForgeChunkManager.Type.values()[packet.readUnsignedByte()];
            if(type == net.minecraftforge.common.ForgeChunkManager.Type.ENTITY)
                entity = world.getEntityByID(packet.readInt());
            int chunks = packet.readUnsignedShort();
    ...
    public class TicketInfoDialog extends JDialog implements LayoutManager
    {
        ...
        public void update()
        {
            ...
            if(ticket.entity != null)
                info += "<br>Entity: " + EntityList.classToStringMapping.get(ticket.entity) + "#" + ticket.entity.entityId + " (" + String.format("%.2f", ticket.entity.posX) + ", " + String.format("%.2f", ticket.entity.posY) + ", " + String.format("%.2f", ticket.entity.posZ) + ")";
            info += "</span><p style=\"text-align:center; font-family:Tahoma; font-size:10px\">ForcedChunks</p><span style=\"font-family:Tahoma; font-size:10px\">";
            for(ChunkCoordIntPair coord : ticket.chunkSet)
                info += coord.chunkXPos + ", " + coord.chunkZPos + "<br>";
            info += "</span>";
        ...
        public void layoutContainer(Container parent)
        {
            ...
            infoPane.setBounds(20, 60, size.width - 40, size.height - 80);
        }
    ...
    private HashMap<Integer, DimensionChunkInfo> dimensionChunks = new HashMap<Integer, DimensionChunkInfo>();
    ...
    {
        instance = new PlayerChunkViewer();
    }

    private PlayerChunkViewer()
    {
        addWindowListener(new WindowAdapter()
        {
            @Override
            public void windowClosing(WindowEvent e)
            {
                if(ClientUtils.mc().getSendQueue() != null)
                    ChunkLoaderClientPacketHandler.sendGuiClosing();
            }
        });
    ...
    protected void startUpdateThread()
    {
        new Thread("Info Frame Update Thread")
        {
            public void run()
            {
                while(true)
                {
                    if(ClientUtils.mc().currentScreen instanceof GuiMainMenu)
                        dispose();
    ...
    protected void update()
    {
        ...
        else
        {
            ...
            {
                if(dims.get(index) != dimComboBox.getItemAt(index))
                {
                    needsReset = true;
                    break;
                }
        ...
        if(xArea == null)
        {
            ...
            xArea.addActionListener(new ActionListener()
            {
                ...
                public void actionPerformed(ActionEvent e)
                {
                    try
                    {
                        Integer i = Integer.parseInt(xArea.getText());
                        if(i != null)
                            setCenter(i, zCenter);
                    }
        ...
        if(zArea == null)
        {
            ...
            zArea.addActionListener(new ActionListener()
            {
                ...
                public void actionPerformed(ActionEvent e)
                {
                    try
                    {
                        Integer i = Integer.parseInt(zArea.getText());
                        if(i != null)
                            setCenter(xCenter, i);
                    }
                    catch(NumberFormatException nfe)
    ...
    public class DisplayArea extends JPanel implements MouseListener, MouseMotionListener
    {
        ...
        public void mouseMoved(MouseEvent event)
        {
            synchronized(dimensionChunks)
            {
                ...
                if(!mouseOverTickets.isEmpty())
                {
                    String tip = "" + mouseOverTickets.size() + (mouseOverTickets.size() == 1 ? " ticket" : " tickets");
